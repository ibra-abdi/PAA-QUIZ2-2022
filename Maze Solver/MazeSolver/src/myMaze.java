...
public class myMaze{
	...
	public void drawMaze(Graphics g) {
		for(int i = 0;i<w;i++) {
			for(int j = 0;j<w;j++) {
				grids[i][j].drawBox(g, new Color(0,255,0,100));
	...
	public void drawPathFinder(Graphics g,int mode) {
		...
		start.drawBox(g,new Color(255, 30, 30));
		end.drawBox(g,new Color(30, 30, 255));
	...
	public void mazeFinderBFS(Graphics g) {
		...
		traceFromAtoB(current,next);
		if(next == end) {
			System.out.println("Finished");
			finish = true;
			createPathStartToEnd();
			return;
		}
	...
	private void createPathStartToEnd() {
	...
	private void traceFromAtoB(Cell A,Cell B) {
		if(B!=null) {
			B.parent = A;
		}
		if(A!=null)
